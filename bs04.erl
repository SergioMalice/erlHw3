...
decode(<<"{", Rest/binary>>, Method) when Method == proplist -> prop(Rest, key, 1, <<>>, [], []).
...
prop(<<"{", Rest/binary>>, Flag, Cnt, Bin, List, Res) -> % { - начинаем парсинг "вложенного json"
  ...
  case (Cnt == 1) of
    true ->
      case Flag of
        inner -> prop(skip(Rest, Count), value, Cnt, Bin, [InnerRes|List], Res);
        _ ->
          case Bin == <<>> of
            false -> prop(skip(Rest, Count), value, Cnt, Bin, [InnerRes|List], Res);
            _ -> prop(skip(Rest, Count), value, Cnt, Bin, List, [InnerRes|Res])
          end
      end;
    _ -> {Bin, InnerRes}
  end;
...
prop(<<Skip, Rest/binary>>, Flag, InnerCnt, Bin, List, Res) when
  (Skip == 32) or % пробел пропускаем
    (Skip == 13) -> % перевод строки пропускаем
  prop(Rest, Flag, InnerCnt, Bin, List, Res);
...
prop(<<_/utf8, Rest/binary>>, Flag, InnerCnt, Bin, List, Res) -> prop(Rest, Flag, InnerCnt, Bin, List, Res);
...
read_inner(<<C, Rest/binary>>, Res, Cnt, InnerCnt) when (C == 32) or (C == 13) -> read_inner(Rest, Res, Cnt, InnerCnt);
read_inner(<<"{", Rest/binary>>, Res, Cnt, InnerCnt) -> read_inner(Rest, <<Res/binary, "{">>, Cnt+1, InnerCnt+1);
read_inner(<<C/utf8, Rest/binary>>, Res, Cnt, InnerCnt) -> read_inner(Rest, <<Res/binary, C/utf8>>, Cnt+1, InnerCnt);
read_inner(<<>>, Res, Cnt, _) -> {Res, Cnt}.

skip(Rest, -1) -> Rest;
skip(<<C, Rest/binary>>, Cnt) when (C == 32) or (C == 13) -> skip(Rest, Cnt);
skip(<<_/utf8, Rest/binary>>, Cnt) -> skip(Rest, Cnt-1).
